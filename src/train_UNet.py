...
def train_model(train_loader, val_loader, file_name, num_epochs=50, device=None):
    ...
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-4)  # Better for segmentation
...
if __name__ == '__main__':
    ...
    MODEL_OUTPUT_FILE = "models/brats_segmentation_model_local_split.pth"
    NUM_EPOCHS = 5
    BATCH_SIZE = 8  # Increased for 2D slices
    RANDOM_STATE = 42
    ...
    loaded_model = UNet(in_channels=4, out_channels=3, features=[64, 128, 256]).to(device)
